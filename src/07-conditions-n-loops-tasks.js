...
function doRectanglesOverlap(rect1, rect2) {
  ...
  if (rect1.left < left2 && left1 > rect2.left && rect1.top < top2 && top1 > rect2.top) return true;
  return false;
}
...
function getDigitalRoot(num) {
  ...
  let res = arr.reduce((acc, el) => Number(acc) + Number(el), 0);
  while (res > 10) {
    res = res.toString().split('').reduce((acc, el) => Number(acc) + Number(el), 0);
  }
  return res;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (start.includes(str[i])) {
      arr.push(str[i]);
    } else if (end.indexOf(str[i]) !== start.indexOf(arr.pop())) {
      return false;
    }
  }
  return !arr.length;
...
function evaluateTicTacToePosition(position) {
  ...
  if (position[1][1]) {
    const firstDiag = position[0][0] === position[1][1] && position[0][0] === position[2][2];
    const secondDiag = position[2][0] === position[1][1] && position[1][1] === position[0][2];
