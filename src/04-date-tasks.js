...
function timeSpanToString(startDate, endDate) {
  ...
  const miliseconds = date.toString().slice(-3);
  return `${hours.toString().padStart(2, '0')}:${minutes.toString().padStart(2, '0')}:${seconds.toString().padStart(2, '0')}.${miliseconds}`;
}
...
function angleBetweenClockHands(date) {
  ...
  const angle = 0.5 * Math.abs(60 * hours - 11 * minutes);
